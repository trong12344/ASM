				...
				<div class="col-sm-8 mt-5 bg-white text-white rounded">
					<div class="container">
						<div class="row pt-3">
							...
							<div class="col-sm-2 mt-2 text-dark fw-bold">ID Sản Phẩm</div>
							<div class="col-sm-2 mt-2 text-dark fw-bold">Tên Sản Phẩm</div>
							<div class="col-sm-2 mt-2 text-dark fw-bold">Giá Bán</div>
							<div class="col-sm-2 mt-2 text-dark fw-bold">Số Lượng</div>
						</div>
								...
								<div class="col-1 text-dark">
									<div class="row">
										<a class="col-3 ms-3 text-dark" style="text-decoration: none;"
											href="/update/iphone-${item.id}/them">+</a> <input
											type="text" value="${item.sl}" class="col-3  form-control"
											id="soluong" name="sl" padding="6px 10px 6px 16px"> <a
											class="col-3 ms-3 text-dark" style="text-decoration: none;"
											href="/update/iphone-${item.id}/xoa">-</a>
									</div>
								</div>
								<a class=" col-1 bi bi-trash text-dark mt-4"
									href="/remove/iphone-${item.id}""></a>
							</div>
							<hr>
						</div>
					</c:forEach> --%>
					<tbody>
						<c:forEach var="item" items="${ cart.getItems() }">
							<input type="hidden" name="id" value="${item.imgProduct}">
							<tr>
								<td><b>${ item.nameProduct }</b> <br> <br> <img
									alt="" src="/image/1.jpeg" width="70%"></td>
								<td>${ item.price }VNĐ</td>
								<td>
									<form action="/cart/update/${item.nameProduct}" method="post">
								...
								<td><a href="/cart/remove/${item.idProduct}"><button
